...
class EqualityBy<E, F> implements Equality<E> {
  ...
  EqualityBy(F Function(E) comparisonKey, [Equality<F> inner = const DefaultEquality<Never>()])
      : _comparisonKey = comparisonKey,
        _inner = inner;

  @override
  bool equals(E e1, E e2) => _inner.equals(_comparisonKey(e1), _comparisonKey(e2));
...
class IterableEquality<E> implements Equality<Iterable<E>> {
  ...
  const IterableEquality([Equality<E> elementEquality = const DefaultEquality<Never>()])
...
abstract class _UnorderedEquality<E, T extends Iterable<E>> implements Equality<T> {
  final Equality<E> _elementEquality;
...
class UnorderedIterableEquality<E> extends _UnorderedEquality<E, Iterable<E>> {
  const UnorderedIterableEquality([Equality<E> elementEquality = const DefaultEquality<Never>()])
...
class SetEquality<E> extends _UnorderedEquality<E, Set<E>> {
  const SetEquality([Equality<E> elementEquality = const DefaultEquality<Never>()]) : super(elementEquality);
...
class MapEquality<K, V> implements Equality<Map<K, V>> {
  ...
  const MapEquality(
      {Equality<K> keys = const DefaultEquality<Never>(), Equality<V> values = const DefaultEquality<Never>()})
      : _keyEquality = keys,
...
class MultiEquality<E> implements Equality<E> {
  ...
  const MultiEquality(Iterable<Equality<E>> equalities) : _equalities = equalities;
...
class DeepCollectionEquality implements Equality {
  ...
  const DeepCollectionEquality.unordered([Equality base = const DefaultEquality<Never>()])
      : _base = base,
        _unordered = true;
